...
my_CPA_2_ = CPA_2o(traces=traces, traces_rout=traces_rout, plains=plains, keys=keys, alphas=alphas)
ge, succ_rate, corr_k_t = my_CPA_2_.compute_ge()
...
plt.ylim(0.4, 600)
# ax.set_title("Guess Entropy and Success Rate under 2nd order CPA (known permutation, rm, unknown rout)")
...
plt.savefig("v2ex_2o_perm.jpg")
plt.show()


fig, ax = plt.subplots(figsize=(15, 7), dpi=500)
# ax.set_title("Maximum value of the correlation coefficient under 2nd order CPA (unknown permutation, rout, known rm)")
ax.set_ylim(0, 1.0)
ax.set_xlabel("能量迹数量")
ax.set_ylabel("相关系数")
ax.plot(corr_k_t, label="相关系数$\\rho_{ck,tk}$")
plt.legend(fancybox=True)
plt.savefig("corr_v2ex_2o_perm_rm.jpg")
plt.show()
